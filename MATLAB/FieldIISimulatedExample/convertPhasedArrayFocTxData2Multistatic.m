...
load phantomFocTxDataPhasedArray.mat; % Cyst and Lesions Phantom
...
tx_delays = zeros(size(txAptPos, 1), numel(theta));
speed_of_sound = c;
...
if strcmp(method, 'Adjoint')
    rf_decoded = refocus_decode(rxdata_multiTx, tx_delays'*fs, ...
        'fun', @Hinv_adjoint, 'param', @(f) 1);
elseif strcmp(method, 'RampFilter')
    rf_decoded = refocus_decode(rxdata_multiTx, tx_delays'*fs, ...
        'fun', @Hinv_adjoint, 'param', @(f) pitch*mean(diff(theta))*f/c);
...
disp(['Correlation of Recovered Multistatic Dataset with Ground Truth is: ', num2str(corr_recon_gnd_truth)]);
