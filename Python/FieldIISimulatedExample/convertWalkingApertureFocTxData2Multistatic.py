...
data_in = sio.loadmat('phantomFocTxDataWalkingAperture.mat'); # Cyst and Lesions Phantom
...
theta = data_in['theta'][0][0];
tx_origin_x = data_in['tx_origin_x'][0];
apod = data_in['apod'];
...
tx_dir = np.array([np.sin(theta), 0, np.cos(theta)]);
...
tx_delays = np.zeros((txAptPos.shape[0], tx_origin_x.size));
speed_of_sound = c;
for kk in np.arange(tx_origin_x.size):
    # Calculate All Geometric Distances
    tx_center = np.array([tx_origin_x[kk], 0.0, 0.0]);
